...
st.subheader("🔥 รีเซ็ตระบบทั้งหมด")

confirm = st.checkbox("เข้าใจว่าข้อมูลจะหายทั้งหมด")

if st.button("ลบไฟล์ฐานข้อมูล"):
    if confirm:
        if delete_db_file():
            st.success("ลบไฟล์ฐานข้อมูลแล้ว กรุณารีเฟรชหน้า")
        else:
            st.warning("ไม่พบไฟล์ฐานข้อมูล")
    else:
        st.error("กรุณายืนยันก่อน")
